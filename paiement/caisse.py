...
def creer_caisse(journaliser):
    ...
    return Caisse(
        clients.Fichier(), prestataire.Prestataire(sequestre, journaliser), journaliser
    )
...
class Caisse:
    ...
    # --- Réservation, départ et règlement ---
        ...
        location.update(
            {
                ...
                "identite": {cle: fiche[cle] for cle in ("prenom", "nom", "telephone")},
    ...
    def partir(self, location, t):
        ...
        self.envoyer(location, DEPART % (numero(location), horaires.texte_heure(t)), t)
    ...
    def terminer(self, location, montant, t):
        ...
        self.envoyer(
            location,
            "Merci %s ! 🏄 Planche %s rendue à %s : %s "
            "débités sur %s. Votre caution de %s est libérée. À la "
            "prochaine fois ! 🌊"
            % (
                location["identite"]["prenom"],
                numero(location),
                horaires.texte_heure(t),
                euros(montant),
                autorisation.libelle,
                euros(autorisation.montant),
            ),
            t,
        )

    def retour_tardif(self, location, t):
        """Confirme le retour physique après la saisie de la caution, sans second règlement."""
        self.envoyer(
            location,
            "La planche %s a bien été rendue à %s. "
            "La caution débitée à 23 h reste acquise."
            % (numero(location), horaires.texte_heure(t)),
            t,
        )
    ...
    def annuler(self, location, t):
        ...
        self.envoyer(
            location,
            "Réservation annulée, %s : rien n'a été débité "
    ...
    def saisir_caution(self, location, t):
        ...
        self.envoyer(
            location,
            "La planche %s n'a pas été rendue avant 23 h : "
            "votre caution de %s a été débitée."
            % (location["balise"], euros(autorisation.montant)),
            t,
        )

    # --- Historique client et présentation ---

    def archiver(self, location, montant, t):
        """Ajoute la location terminée à la fiche du client."""
        self.fichier.ajouter_location(
            location["identite"]["telephone"],
            {
                "planche": location["balise"],
                "montant": round(montant, 2),
                "moyen": location["autorisation"].libelle,
                "t": t,
    ...
    def envoyer(self, location, texte, t):
        ...
        location["messages"].append({"heure": horaires.texte_heure(t), "texte": texte})
    ...
    def resume(self, location):
        ...
        return {
            ...
            "paiement": {"type": autorisation.moyen, "libelle": autorisation.libelle},
            "caution": {
                "montant": autorisation.montant,
