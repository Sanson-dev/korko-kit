...
def decoder_image(data_url):
    ...
    if not separateur or not entete.startswith("data:") or not entete.endswith(";base64"):
        raise PhotoErreur("Format de photo invalide. Choisissez une autre image.")
    mime = entete[5:-7].lower()
    if mime not in MIME_TYPES:
        raise PhotoErreur("Type de photo non pris en charge. Utilisez JPEG, PNG ou WebP.")
    if len(contenu) > 4 * ((MAX_IMAGE_BYTES + 2) // 3):
        raise PhotoErreur("La photo dépasse 8 Mo. Choisissez une image plus légère.", 413)
    try:
        donnees = base64.b64decode(contenu, validate=True)
    except (ValueError, binascii.Error):
        raise PhotoErreur("Le fichier image est illisible. Réessayez avec une autre photo.")
    if not donnees:
        raise PhotoErreur("La photo est vide. Choisissez une autre image.")
    if len(donnees) > MAX_IMAGE_BYTES:
        raise PhotoErreur("La photo dépasse 8 Mo. Choisissez une image plus légère.", 413)
    signatures = {
        "image/jpeg": donnees.startswith(b"\xff\xd8\xff") and donnees.endswith(b"\xff\xd9"),
    ...
    if not signatures[mime]:
        raise PhotoErreur("Le contenu du fichier ne correspond pas à une image valide de ce type.")
    return mime, donnees
...
SCHEMA = {
    ...
    "properties": {
        ...
        "etat": {"type": "string", "enum": ["sans_defaut_visible", "defaut_visible", "incertain"]},
        "defaut": {"type": "string"},
        "certitude": {"type": "string", "enum": ["elevee", "faible"]},
    },
    "required": ["planche_visible", "photo_exploitable", "numero_lisible", "numero_lu",
                 "etat", "defaut", "certitude"],
...
def analyser_image(mime, donnees):
    ...
    if not cle:
        raise PhotoErreur("L'analyse photo n'est pas configurée sur le serveur (OPENAI_API_KEY).", 503)
    ...
    corps = {
        ...
        "text": {"format": {"type": "json_schema", "name": "korko_photo_retour",
                             "strict": True, "schema": SCHEMA}},
    }
    requete = urllib.request.Request(
        API_URL, json.dumps(corps).encode("utf-8"),
    ...
    try:
        ...
        if erreur.code in (401, 403):
            raise PhotoErreur("La clé du service d'analyse photo est refusée. Vérifiez OPENAI_API_KEY.", 503)
        if erreur.code == 429:
            try:
            ...
            if code in ("insufficient_quota", "billing_hard_limit_reached"):
                raise PhotoErreur("Le quota ou les crédits de l'API OpenAI sont épuisés. Vérifiez la facturation du compte API.", 503)
            raise PhotoErreur("Le service d'analyse photo limite temporairement les demandes.", 503,
                              retryable=True, retry_after=_retry_after(erreur.headers))
        if erreur.code in (408, 425) or 500 <= erreur.code <= 599:
            raise PhotoErreur("Le service d'analyse photo est temporairement indisponible.", 503,
                              retryable=True, retry_after=_retry_after(erreur.headers))
        raise PhotoErreur("Le service d'analyse photo a refusé la demande (HTTP %d)." % erreur.code, 502)
    except (urllib.error.URLError, TimeoutError, socket.timeout, OSError):
        raise PhotoErreur("Connexion au service d'analyse impossible. Un nouvel essai sera effectué.", 504,
                          retryable=True)
    ...
    try:
        ...
        textes = [partie["text"] for item in reponse.get("output", [])
                  if item.get("type") == "message"
                  for partie in item.get("content", []) if partie.get("type") == "output_text"]
        if len(textes) != 1:
            raise ValueError("texte absent")
        observation = json.loads(textes[0])
        if not isinstance(observation, dict) or set(observation) != set(SCHEMA["properties"]):
            raise ValueError("schéma inattendu")
        if any(type(observation[cle]) is not bool for cle in
               ("planche_visible", "photo_exploitable", "numero_lisible")):
            raise ValueError("booléens invalides")
        if any(not isinstance(observation[cle], str) for cle in
               ("numero_lu", "etat", "defaut", "certitude")):
            raise ValueError("chaînes invalides")
        if (observation["etat"] not in SCHEMA["properties"]["etat"]["enum"] or
                observation["certitude"] not in SCHEMA["properties"]["certitude"]["enum"]):
            raise ValueError("valeur inattendue")
    except (ValueError, TypeError, KeyError, AttributeError, UnicodeDecodeError):
        raise PhotoErreur("L'analyse n'a pas fourni de résultat fiable. Réessayez la photo.", 502)
    return observation
...
def classer(observation, planche_attendue):
    ...
    if not observation["planche_visible"]:
        return resultat("reprendre", "Reprenez une photo",
    ...
    numero = numero_normalise(observation["numero_lu"]) if observation["numero_lisible"] else None
    if numero is None:
        return resultat("reprendre", "Reprenez une photo",
    ...
    if numero != planche_attendue:
        ...
        return resultat("autre_planche", "Ce n’est pas la planche louée",
    ...
    if observation["etat"] == "defaut_visible":
        ...
        return resultat("abimee", "La planche semble abîmée", "Défaut visible : " + defaut)
    if observation["etat"] == "sans_defaut_visible":
        return resultat("conforme", "La planche est conforme",
