(() => {
  let id = localStorage.getItem('korko-session') || (crypto.randomUUID ? crypto.randomUUID() : String(Date.now()) + Math.random()), poller, receiptSession, dismissedSession;
  ...
  function render(d){if(d.etat==='armée'){$('board-name').textContent=board(d.balise);show('armed')}if(d.etat==='en cours'){$('active-board').textContent=board(d.balise);$('board-update').textContent=d.message||'';$('board-update').hidden=!d.message;$('duration').textContent=duration(d.duree);$('current-price').textContent=money(d.montant);show('active')}if(d.etat==='retournée'){$('receipt-board').textContent=board(d.balise);$('final-duration').textContent=duration(d.duree);$('final-price').textContent=money(d.montant);if(receiptSession!==d.session_id){receiptSession=d.session_id;$('condition-error').textContent=''}$('condition-form').hidden=!!d.rapport_condition||dismissedSession===d.session_id;$('condition-thanks').hidden=!d.rapport_condition;show('receipt')}}
  ...
  document.querySelectorAll('[data-condition]').forEach(button=>button.addEventListener('click',async()=>{const choices=[...document.querySelectorAll('[data-condition]')];choices.forEach(b=>{b.disabled=true;b.classList.toggle('selected',b===button)});$('condition-error').textContent='';try{const r=await fetch('/api/condition',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({identifiant:id,session_id:receiptSession,condition:button.dataset.condition,photo:$('photo-input').files[0]?.name||null})}),d=await r.json();if(!r.ok)throw Error(d.erreur);$('condition-form').hidden=true;$('condition-thanks').hidden=false}catch(_){choices.forEach(b=>b.disabled=false);$('condition-error').textContent='Envoi impossible. Vous pouvez réessayer ou passer.'}}));
