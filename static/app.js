...
function afficherRecuLocal(location) {
  ...
  element('verification-photo').hidden = true;
  afficherEcran('recu');
}
...
function afficherRecu(donnees) {
  ...
  element('verification-photo').hidden = false;
  ...
  if (!vue.photoEnvoi && vue.photoRetrySession !== donnees.session_id) {
    afficherTachePhoto(donnees.photo_task);
  }
  afficherEcran('recu');
}
...
function erreurPhoto(message) {
  ...
  element('photo-error').hidden = !message;
}
...
function effacerPhoto(garderFichier = false) {
  ...
  element('submit-photo').disabled = true;
  element('photo-progress').hidden = true;
...
function afficherResultatPhoto(resultat) {
  ...
  element('photo-result-title').textContent = resultat.titre || 'Résultat de la photo';
  element('photo-result-message').textContent = resultat.message || '';
  element('retry-photo').hidden = resultat.statut === 'conforme';
  element('retry-photo').textContent = 'Prendre une autre photo';
  element('photo-pending').hidden = true;
  bloc.hidden = false;
  element('submit-photo').hidden = true;
  element('bouton-photo').hidden = true;
  return true;
}
...
function afficherTachePhoto(tache) {
  if (!tache) {
    element('photo-pending').hidden = true;
    element('photo-result').hidden = true;
    element('bouton-photo').hidden = false;
    return false;
  }
  ...
  if (tache.status === 'done' && afficherResultatPhoto(tache.result)) {
    vue.photoEchec = false;
    return true;
  }
...
function surPhotoChoisie() {
  ...
  if (!fichier.size || fichier.size > 8 * 1024 * 1024) {
    erreurPhoto('La photo doit faire 8 Mo maximum.');
    return;
  }
  vue.photoTaskId = Array.from(crypto.getRandomValues(new Uint8Array(16)),
    octet => octet.toString(16).padStart(2, '0')).join('');
  vue.photoEchec = false;
  vue.photoPreviewUrl = URL.createObjectURL(fichier);
  element('photo-preview').src = vue.photoPreviewUrl;
  element('photo-preview').hidden = false;
  element('note-photo').textContent = 'Photo choisie : ' + fichier.name;
  element('submit-photo').hidden = false;
  element('submit-photo').disabled = false;
}
...
async function verifierServeurPhoto() {
  const capacites = await appelerApi('/api/capabilities');
  if (capacites.photo_verification !== true || capacites.version !== 'photo-queue-1') {
    throw new Error('Le cloud en cours est trop ancien. Relancez mon_cloud.py.');
  }
}
...
async function envoyerPhoto() {
  ...
  vue.photoEnvoi = true;
  erreurPhoto('');
  element('photo-progress').textContent = 'Envoi de la photo en cours…';
  element('photo-progress').hidden = false;
  element('submit-photo').disabled = true;
  element('submit-photo').textContent = 'Envoi…';
  element('bouton-photo').disabled = true;
  try {
    ...
    if (!erreur.ignoree && epoque === vue.epoque && sessionId === vue.sessionRecu) {
      erreurPhoto(erreur.message || 'Envoi impossible. Réessayez.');
    }
  } finally {
    vue.photoEnvoi = false;
    if (epoque === vue.epoque && sessionId === vue.sessionRecu) {
      element('photo-progress').hidden = true;
      element('submit-photo').textContent = 'Envoyer la photo';
      element('submit-photo').disabled = !element('photo').files[0];
      element('bouton-photo').disabled = false;
    }
  }
}

function retenterPhoto() {
  vue.photoRetrySession = vue.sessionRecu;
  element('photo-result').hidden = true;
  element('photo-pending').hidden = true;
  element('bouton-photo').hidden = false;
  if (vue.photoEchec && element('photo').files[0]) {
    element('submit-photo').hidden = false;
    element('submit-photo').disabled = false;
  } else {
    effacerPhoto();
...
function brancherRapport() {
  ...
  element('submit-photo').addEventListener('click', envoyerPhoto);
  element('retry-photo').addEventListener('click', retenterPhoto);
