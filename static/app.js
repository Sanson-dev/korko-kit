(() => {
  let id = localStorage.getItem('korko-session') || (crypto.randomUUID ? crypto.randomUUID() : String(Date.now()) + Math.random()), poller, receiptSession, dismissedSession, hadSession = false, generation = null, resetDetected = false, armSubmitting = false, stateRequest = 0, cloudAvailable = true;
  ...
  async function park(){try{const d=await (await fetch('/api/parc')).json();$('board-count').textContent=Object.values(d.planches).filter(p=>p.ou==='A'&&p.statut==='au râtelier').length;cloudAvailable=true}catch(_){cloudAvailable=false;try{const r=await fetch(LOCAL_API+'/offline/status');const d=await r.json();$('board-count').textContent=d.planches_disponibles.length}catch(_){$('board-count').textContent='—'}updateOfflineControls()}}
  ...
  try{const cached=JSON.parse(localStorage.getItem('korko-customer-state')||'null');if(cached&&cached.id===id){phone=cached.phone||phone;hadSession=true;render(cached.state)}}catch(_){}
