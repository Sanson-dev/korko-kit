...
DB_PATH = os.environ.get(
    "KORKO_CLOUD_DB",
    os.path.join(os.path.dirname(os.path.abspath(__file__)), "korko_cloud.sqlite3"),
)
...
SANS_PAIEMENT = {
    "paiement": {"type": "aucun", "libelle": "aucun moyen enregistré"},
...
def autorisation_hors_ligne(identifiant, station):
    """Signe un droit de réservation local, expirant selon le temps réel."""
    payload = json.dumps(
        {
...
def sauvegarder():
    ...
    """Persiste un instantané métier et les identifiants déjà reçus dans SQLite."""
    data = {
        k: globals()[k]
        for k in (
            "planches",
            ...
            "horloge",
            "demo_generation",
            "rebalancements_actifs",
            "historique_etrangeres",
        )
    }
    ...
    with sqlite3.connect(DB_PATH, timeout=10) as db:
        db.execute(
            "CREATE TABLE IF NOT EXISTS etat (id INTEGER PRIMARY KEY CHECK(id=1), data TEXT NOT NULL)"
        )
        db.execute(
            "INSERT INTO etat(id,data) VALUES(1,?) ON CONFLICT(id) DO UPDATE SET data=excluded.data",
            (texte,),
        )
...
def charger():
    ...
    """Restaure les états métier et reconstruit les autorisations de paiement."""
    ...
    try:
        ...
        horloge = data.get("horloge", 0.0)
        demo_generation = data.get("demo_generation", 0)
        ...
        for experience in client_sessions.values():
            if isinstance(experience.get("autorisation"), dict):
                experience["autorisation"] = prestataire.Autorisation.depuis(
                    experience["autorisation"]
                )
...
def reinitialiser_demo():
    ...
    for experience in client_sessions.values():
        ...
        if (
            experience.get("etat") in ("armée", "en cours")
            and "autorisation" in experience
        ):
            CAISSE.prestataire.debiter(experience["autorisation"], 0.0)
    # Les lectures photo filtrent cette génération : un ancien résultat ne
    # doit pas réapparaître dans les sessions de la nouvelle démonstration.
    demo_generation += 1
...
# --- Présentation du temps et journalisation ---
...
# --- Réservations et retours de location ---


def suggestion(station):
    """Propose, parmi les planches disponibles ici, celle qui est le moins sortie."""
    dispo = [
        b
...
def armer(client, moyen, station, identifiant):
    ...
    etat = {
        ...
        "etat": "armée",
        "armee_a": horloge,
        "reservation_t": horloge,
        "reservation_status": "Réservée",
...
def annuler(identifiant, expiree=False):
    ...
    note(
        "%s : réservation de %s %s"
        % (experience["client"], balise, "expirée" if expiree else "annulée")
    )
    return None


def cloturer(balise, station, t, hors_base):
    """Traite le retour physique, y compris après perte ou départ sans client."""
    p = planches[balise]
    ...
    if not s:
        ...
        for identifiant, experience in list(client_sessions.items()):
            if experience.get("balise") == balise and experience.get("etat") in (
                "retournée",
                "retour_tardif",
                "terminée",
            ):
                note(
                    "RETOUR DUPLIQUÉ : %s déjà clôturée, aucun changement appliqué"
                    % balise
                )
                reservations.pop(balise, None)
                return
    ...
    if experience:
        if experience.get("etat") in ("retournée", "retour_tardif", "terminée"):
            return note(
                "RETOUR DUPLIQUÉ : %s déjà clôturée pour %s" % (balise, identifiant)
            )
        experience.update(
            {
                "etat": "retournée",
                ...
                "duree": duree,
                "montant": montant,
                "retour_station": station,
                "reservation_status": "Terminée",
            }
        )
        # Une autre planche a pu être prise que celle réservée. Au retour,
        # libérer toutes les associations de réservation de ce client.
    ...
    if experience and "autorisation" in experience:
        # La saisie a déjà réglé la caution : le retour tardif ne débite rien.
        ...
        sms(
            s["client"], "Merci ! %s, %s, %.2f €." % (balise, duree_txt(duree), montant)
        )
    if hors_base:
        note(
            "RÉÉQUILIBRAGE : %s rendue en %s, sa base est %s"
...
# --- Planches étrangères, rapports et remise en service ---
...
def signaler_etrangere(balise, station, t, event_id):
    ...
    if station == origine:
        return note(
            "ETRANGERE incohérente ignorée : %s détectée à sa station %s"
            % (balise, station)
        )
    ...
    if balise not in rebalancements_actifs:
        ...
        note(
            "ETRANGERE : %s, base %s, détectée en %s (t=%.1f)"
            % (balise, origine, station, t)
        )


def confirmer_rebalancement(balise):
    """Confirme le retour à la base d’une planche sans location active."""
    ...
    if p["statut"] == "A_REEQUILIBRER":
        p["statut"] = "au râtelier"
    note(
        "RÉÉQUILIBRAGE CONFIRMÉ : %s rétablie à sa station %s" % (balise, p["origine"])
    )
    return "Rééquilibrage confirmé", None


def enregistrer_rapport(identifiant, session_id, condition, photo=None):
    """Lie un rapport au retour concerné et adapte inspection ou maintenance."""
    experience = client_sessions.get(identifiant)
    if (
        not experience
        or experience.get("etat") != "retournée"
        or experience.get("session_id") != session_id
    ):
    ...
    note(
        "ÉTAT DE PLANCHE : %s — %s (retour t=%.0f)"
        % (rapport["balise"], condition, rapport["t"])
    )
    return rapport, None


def reparer(balise):
    """Rend la planche disponible à sa base, ou attend son retour physique."""
    ...
    if planche.get("ou") == planche["origine"] and balise not in sessions:
        ...
        note(
            "RÉPARATION VALIDÉE : %s déjà présente à son râtelier %s"
            % (balise, planche["origine"])
        )
        return "Disponible", None
    note(
        "RÉPARATION TERMINÉE : %s en attente de son râtelier %s"
        % (balise, planche["origine"])
    )
    return "Réparation terminée — en attente du retour au râtelier", None


# --- Échéances métier et reprise des réservations hors ligne ---
...
def reserver_hors_ligne(ev, station):
    ...
    identite = (
        {cle: fiche[cle] for cle in ("prenom", "nom", "telephone")} if fiche else {}
    )
    try:
        CAISSE.reserver(
            etat, dict(identite, appareil=identifiant), {"type": "enregistre"}, ev["t"]
        )
    except caisse.REFUS as refus:
...
def traiter(ev):
    """Applique un événement de station en conservant son horodatage métier."""
    global horloge
    ...
    # Les délais suivent t ; monotonic sert uniquement à détecter les
    # stations muettes, indépendamment de la vitesse du simulateur.
    if t < signes.get(station, t) - 60:
        horloge = t  # simulateur relancé : son horloge repart de 9 h
    horloge = max(horloge, t)
    if station not in signes:
    ...
    if type_ == "DEPART":
        ...
        if balise in sessions or p["statut"] in ("en mer", "départ ambigu"):
            return note(
                "ALERTE : départ dupliqué ignoré pour %s, déjà associé à une session"
                % balise
            )
        ...
        if (
            experience
            and experience["etat"] == "armée"
            and experience["station"] == station
        ):
            reservations.pop(balise, None)
            ...
            experience.update(
                {
                    "etat": "en cours",
                    "reservation_status": "Consommée",
                    "depart_a": ev["t"],
                }
            )
        elif (
            not identifiant
            and p["statut"] == "au râtelier"
            and p["ou"] == station
            and balise not in sessions
        ):
            # Réattribuer la planche réellement prise seulement si un seul
            # client est identifiable ; sinon, garder le départ ambigu.
            candidates = [
                (i, e)
                for i, e in client_sessions.items()
            ...
            if len(candidates) == 1 and not legacy:
                ...
                if (
                    ancien in reservations
                    and reservations[ancien] == identifiant
                    and ancien not in sessions
                    and planches[ancien]["statut"] == "réservée"
                ):
                    ...
                    experience.update(
                        {
                            "balise": balise,
                            "ecart": "Assigned %s -> actual departure %s"
                            % (ancien, balise),
                            "message": "Vous avez pris la planche %s. Aucun problème, votre session a été mise à jour."
                            % balise.replace("korko-", ""),
                        }
                    )
                    note(
                        "PLANCHE MISE À JOUR : %s -> actual departure %s (%s)"
                        % (ancien, balise, experience["client"])
                    )
                    reservations.pop(balise, None)
                    client = experience["client"]
                    experience.update(
            ...
            if len(candidates) > 1 or (candidates and legacy):
                ...
                experience = None
                note(
                    "AMBIGUÏTÉ : départ de %s en %s, %d sessions armées et %d client(s) historique(s)"
                    % (balise, station, len(candidates), len(legacy))
                )
            elif not candidates and legacy:
                client, identifiant = legacy.pop(0), None
            elif not candidates:
                # Le mouvement physique ne suffit pas à inventer une location.
            ...
            return note(
                "ALERTE : départ de %s ignoré, planche indisponible ou réservée"
                % balise
            )
        ...
        event_id = (
            ...
            or hashlib.sha256(
                "\0".join(
                    str(ev.get(k, "")) for k in ("station", "evenement", "balise", "t")
                ).encode("utf-8")
            ).hexdigest()
        )
...
# --- Vues client et administration ---


def client_json(identifiant):
    """Construit la vue client, avec reprise du reçu depuis la file photo."""
    e = client_sessions.get(identifiant)
    commun = {
        "t": horloge,
        "heure": horaires.texte_heure(horloge),
...
def tableau():
    """Résume les stations, les planches et les écarts pour l’administration."""
    lignes = ["STATIONS", "--------"]
    lignes += [
        "%s   %s / dernière nouvelle t=%.0f s"
        % (
            st,
            (
                "Connectée"
                if time.monotonic() - dernier_contact.get(st, 0) < 5
                else "En retard"
            ),
            t,
        )
        for st, t in sorted(signes.items())
    ] or ["Aucune station branchée"]
    lignes += ["", "PARC", "----"]
    for b, p in sorted(planches.items()):
        ...
        reservation = next(
            (
                e
                for e in reversed(list(client_sessions.values()))
                if e.get("balise") == b
                and e.get("reservation_status") in ("Réservée", "En cours", "Expirée")
            ),
            None,
        )
        if reservation:
    ...
    ecarts = [
        "%s — %s" % (e["client"], e["ecart"])
        for e in client_sessions.values()
        if e.get("ecart")
    ]
    ambigu = [
        "Départ ambigu : %s" % b
        for b, s in sessions.items()
        if s.get("identifiant") is None and s["client"].startswith("Départ ambigu")
    ]
    lignes += ecarts + ambigu if ecarts or ambigu else ["Aucun"]
    return "\n".join(lignes + ["", "JOURNAL", "-------"] + journal[:15])


# --- Serveur HTTP : transport, photo, commandes et consultation ---


class Cloud(BaseHTTPRequestHandler):
    """Sert les API et les pages ; sérialise les requêtes sur l’état partagé."""
    ...
    def json(self, objet, code=200):
        self.repondre(
            json.dumps(objet, ensure_ascii=False),
    ...
    def verifier_photo(self):
        ...
        if (
            ...
            return self.json(
                {"erreur": "Envoi invalide. Réessayez avec une photo."}, 415
            )
        try:
        ...
        if taille > photo.MAX_REQUEST_BYTES:
            return self.json(
                {"erreur": "La photo dépasse 8 Mo. Choisissez une image plus légère."},
                413,
            )
        try:
            ...
            return self.json(
                {"erreur": "L'envoi de la photo est illisible. Réessayez."}, 400
            )
        ...
        if not isinstance(task_id, str):
            return self.json(
                {"erreur": "Identifiant de tâche manquant. Réessayez l'envoi."}, 400
            )
        ...
        with state_lock:
            ...
            if (
                ...
                or experience.get("etat") != "retournée"
                or experience.get("session_id") != session_id
                or donnees.get("generation") != demo_generation
            ):
                return self.json(
                    {
                        "erreur": "Cette session n'est plus disponible. Actualisez la page."
                    },
                    409,
                )
        ...
        # Le décodage se fait hors du verrou : revérifier ensuite la session
        # et la génération avant d’accepter durablement la photo.
        try:
            ...
            with state_lock:
                ...
                if (
                    ...
                    or not current
                    or current.get("etat") != "retournée"
                    or current.get("session_id") != session_id
                    or current.get("balise") != planche_attendue
                ):
                    return self.json(
                        {
                            "erreur": "La session a changé pendant l'envoi. Actualisez la page."
                        },
                        409,
                    )
            ...
            photo_note(
                "PHOTO %s : mise en file impossible (%s)"
                % (planche_attendue, type(erreur).__name__)
            )
            return self.json(
                {"erreur": "Le cloud n'a pas pu enregistrer la photo. Réessayez."}, 500
            )
    ...
    def traiter_post(self):
        ...
        if u.path == "/api/condition":
            ...
            rapport, erreur = enregistrer_rapport(
                str(d["identifiant"]), str(d["session_id"]), d.get("condition"), photo
            )
            sauvegarder()
            return self.json(
                (
                    {"erreur": erreur, "generation": demo_generation}
                    if erreur
                    else {"rapport": rapport, "generation": demo_generation}
                ),
                409 if erreur else 200,
            )
        if u.path == "/api/reparer":
            ...
            sauvegarder()
            return self.json(
                {"erreur": erreur} if erreur else {"statut": statut},
                409 if erreur else 200,
            )
        ...
        if u.path == "/api/admin/reset-demo":
            ...
            reinitialiser_demo()
            return self.json(
                {"message": "Cloud réinitialisé.", "generation": demo_generation}
            )
        if u.path == "/api/arme":
            d = self.lire_json()
            if (
                not d
                or not d.get("identifiant")
            ...
            if "generation" in d and d["generation"] != demo_generation:
                return self.json(
                    {
                        "erreur": "La démonstration a été réinitialisée.",
                        "generation": demo_generation,
                    },
                    409,
                )
            ...
            etat["offline_authorization"] = autorisation_hors_ligne(
                identifiant, station
            )
                ...
                for ev in events:
                    ...
                    event_id = (
                        ...
                        or hashlib.sha256(
                            "\0".join(
                                str(ev.get(k, ""))
                                for k in ("station", "evenement", "balise", "t")
                            ).encode("utf-8")
                        ).hexdigest()
                    )
        ...
        if u.path == "/admin":
            alertes = (
                "".join(
                    ...
                    % (
                        html.escape(e["balise"]),
                        html.escape(e["origine"]),
                        html.escape(e["station"]),
                        e["t"],
                        html.escape(e["balise"], quote=True),
                    )
                    for e in rebalancements_actifs.values()
                )
                or "<li>Aucune planche à rééquilibrer</li>"
            )
            rapports = (
                "".join(
                    "<li class='%s'>%s | %s | retour t=%.0f%s</li>"
                    % (
                        "damaged" if r["condition"] == "DAMAGED" else "",
                        html.escape(r["balise"]),
                        html.escape(r["condition"]),
                        r["t"],
                        (
                            " | photo prototype: " + html.escape(r["photo"])
                            if r.get("photo")
                            else ""
                        ),
                    )
                    for r in rapports_planches[:10]
                )
                or "<li>Aucun retour reçu</li>"
            )
            items = []
            for b, p in sorted(planches.items()):
                ...
                reservation = next(
                    (
                        ...
                        for e in reversed(list(client_sessions.values()))
                        if e.get("balise") == b
                        and e.get("reservation_status")
                        in ("Réservée", "En cours", "Expirée")
                    ),
                    None,
                )
                if p["statut"] == "A_REEQUILIBRER":
                    status = "À rééquilibrer — détectée à %s (base %s)" % (
                        p.get("ou"),
                        p["origine"],
                    )
                elif p["statut"] == "maintenance":
                    status = (
                        "Réparation terminée — en attente du retour au râtelier"
                        if p.get("reparation_demandee")
                        else "En maintenance"
                    )
                    ...
                    status = p["statut"]
                detail = (
                    "<br><small>Dernier état : %s%s</small>"
                    % (
                        html.escape(latest["condition"]),
                        (
                            " — " + html.escape(latest.get("photo", ""))
                            if latest.get("photo")
                            else ""
                        ),
                    )
                ...
                if reservation:
                    detail += "<br><small>Réservation : %s</small>" % html.escape(
                        reservation["reservation_status"]
                    )
                action = (
                    " <button type='button' onclick=\"reparer('%s')\">Réparée — remettre en service</button>"
                    % html.escape(b, quote=True)
                    if p["statut"] == "maintenance" and not p.get("reparation_demandee")
                    else ""
                )
                items.append(
                    "<li><strong>%s</strong> — %s%s%s</li>"
                    % (html.escape(b), html.escape(status), detail, action)
                )
            fiches = (
                "".join("<li>%s</li>" % html.escape(ligne) for ligne in CAISSE.fiches())
                or "<li>Aucun client</li>"
            )
            page = (
                (
                    "<!doctype html>"
                    ...
                    "KORKO admin"
                    "</title>"
                    "<body style='font:14px ui-monospace,monospace;background:#ede3ce;color:#164b55;padding:24px'>"
                    "<h2>"
                    "KORKO · administration · %s · t = %.0f s"
                    "</h2>"
                    ...
                    "%s"
                    "</pre>"
                    "<section style='margin-top:32px;padding:18px;border:2px solid #bd744c;border-radius:12px;background:#fffaf0'>"
                    "<h3>"
                    "DÉMO"
                    "</h3>"
                    "<button id='reset-start' type='button'>"
                    "Réinitialiser la démo"
                    "</button>"
                    "<div id='reset-confirm' hidden>"
                    "<p>"
                    "Réinitialiser toutes les données temporaires de la démo ?"
                    "</p>"
                    "<p>"
                    "Les locations, réservations et états temporaires seront effacés."
                    "</p>"
                    "<button id='reset-cancel' type='button'>"
                    "Annuler"
                    "</button>"
                    " "
                    ...
                    "</div>"
                    "<p>"
                    "Efface les locations, réservations, sessions clients, files d'attente, "
                    "rapports de condition, alertes/journal, compteurs de sorties et signaux "
                    "de stations. Les états de maintenance et d'inspection sont remis à zéro. "
                    "Les cautions encore bloquées sont libérées. "
                    "Aucun fichier ni simulateur physique n'est modifié."
                    "</p>"
                    "<p id='reset-message' role='status' aria-live='polite'>"
                    "</p>"
                    "</section>"
                    """<style>
.damaged {
...
button {
  ...
  cursor: pointer
                    ...
                    """<script>
async function reparer(b) {
  const r = await fetch('/api/reparer', {
    method: 'POST',
    headers: {
...
async function rebalancer(b) {
  const r = await fetch('/api/rebalancer', {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json'
    },
...
const start = document.getElementById('reset-start'),
  box = document.getElementById('reset-confirm'),
  msg = document.getElementById('reset-message');
start.onclick = () => {
  box.hidden = false;
  start.disabled = true
};
...
document.getElementById('reset-submit').onclick = async () => {
  const b = document.getElementById('reset-submit');
  b.disabled = true;
  try {
    ...
    const d = await r.json();
    if (!r.ok)
      throw Error(d.erreur);
    sessionStorage.setItem('korko-reset-message', JSON.stringify({
      message: d.message,
...
const saved = JSON.parse(sessionStorage.getItem('korko-reset-message') || 'null');
...
refresh();
        ...
        if u.path in ("/", "/index.html"):
            return self.servir("index.html", "text/html; charset=utf-8")
        if u.path in ("/static/style.css", "/static/app.js"):
            return self.servir(
    ...
    def servir(self, nom, type_):
        try:
            with open(os.path.join(STATIQUE, nom), "rb") as f:
                self.repondre(f.read(), type_)
        except OSError:
            self.repondre("Fichier statique introuvable", code=404)
