...
class FilePhotos:
    ...
    def latest_for_client(self, client_id, session_id=None):
    ...
    def session_snapshot(self, client_id):
        with self._database() as db:
            row = db.execute("SELECT * FROM photo_tasks WHERE client_id=? ORDER BY rowid DESC LIMIT 1",
                             (client_id,)).fetchone()
        if not row:
            return None
